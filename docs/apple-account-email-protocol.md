...
首次使用时先复制当前 iCloud 登录中的 Cookie 和认证上下文，读取账户管理站点及 bootstrap，使用其 serviceKey 发起授权，再初始化管理会话并校验返回的账户身份。已有管理会话优先直接使用；被 Apple 拒绝时再尝试从当前 iCloud 登录恢复一次。此过程只发起读取请求，不提交密码或触发设备验证码。Cookie 按原有域名和路径匹配，不把 iCloud 服务 Cookie 强行发送到账户管理站点。

只有 Apple 返回明确的认证拒绝（HTTP 401/403）时才显示重新登录。网络错误、限流、服务异常、响应格式错误和账户身份不匹配不会触发密码登录。恢复后的管理会话单独加密保存，不覆盖原 iCloud 会话；Apple 实际是否允许免密码授权取决于现有会话。需要重新认证时，可能还需输入设备验证码。添加邮箱的验证码与设备验证码分属两个不同验证流程。
