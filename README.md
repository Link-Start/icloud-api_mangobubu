...
- “转发设置”支持添加 Apple 账户电子邮箱并通过邮箱验证码确认，也可删除备用邮箱；仅剩一个可转发邮箱时禁止删除。操作成功后自动刷新转发列表。管理邮箱会先尝试复用已有 Apple 登录，只有 Apple 明确要求重新认证时才提示登录。接口与验证流程见 [Apple 账户电子邮箱管理](docs/apple-account-email-protocol.md)。
- 在“主号管理 → 管理”中使用“转发设置”，获取 Apple 可用的转发邮箱、单选目标并保存；未登录时会先完成 Apple 登录。此设置作用于该 Apple 账户的隐私邮箱，不会修改本地主号或 IMAP 配置。自动创建、目录同步、手动登记和删除使用 Apple 当前选定的转发目标，支持第三方邮箱，不要求转发地址等于主号邮箱或最终 IMAP 用户名。
...
自动创建日志中的 HTTP 200 仅表示请求完成；Apple 明确返回 `success:false` 时仍记录为业务失败。此时不会仅因进入创建步骤就提示“可能已产生远端变更”，但本轮此前尝试修改转发目标、待确认候选等状态仍会保留。“当前请求可重试”表示客户端是否允许直接重发本次请求；创建提交失败后不直接重发，后续执行以“计划动作”和“下次执行”时间为准。连接中断或响应不完整时，仍通过保存候选并核对目录来确认结果。

自动创建失败详情中的“查看原始信息”可展开 Apple 隐私邮箱接口的脱敏响应，保留原业务码、错误说明及对应请求的 HTTP 状态；复制完整流程也包含这些信息。响应快照在进入日志前移除登录凭据和敏感身份字段，正文最多保留 1500 字节，截断时显示标记及已读取的响应字节数。不完整 JSON 等未保留正文的情况会显示原因，登录接口正文和认证头不进入快照。旧日志未保存原始响应，升级后新产生的失败记录才包含快照；管理页沿用现有日志缓冲区保留快照，部署日志同步记录脱敏内容。

在“日志”页选择“创建”类别，可集中查看自动创建和待确认目录核对的流程记录，并结合主号、创建编号、级别和关键词追查。流程中的 `auto_create_kind` 区分本轮“新建”（`new`）、“复查”（`reconcile`）和“尚未判定”（`undetermined`），历史缺少该字段的记录显示“未记录”。管理端 `GET <admin-path>/api/v1/logs?category=creation` 使用结构化 `auto_create_run_id` 筛选创建日志，兼容已有带创建编号的记录；类别为空时返回全部日志。类别筛选先于分页，返回的 `pagination.total` 为当前所有筛选条件下的日志总数。
...
以上三套规则只决定邮件取件与归属。Apple 隐私邮箱自动创建、目录同步、手动登记和删除均以实时目录中的 `selectedForwardTo` 为转发目标，允许它是第三方邮箱或中间转发地址，无需等于本地主号邮箱或最终 IMAP 用户名。Apple 登录账户身份仍通过已保存会话中的 Apple ID 和 DSID 校验；自动创建还会核对创建响应和完整目录中的地址、启用状态及转发目标。已有地址操作在旧目录未返回 `selectedForwardTo` 时沿用主号邮箱作为兼容目标；新建必须先取得明确的默认转发设置，不会覆盖已经选定的第三方目标。请将 IMAP 主机、用户名和密码配置为最终收件邮箱，转发设置本身不会自动修改这些配置。
