...
自动创建时，“待确认”只表示创建结果尚未确认，不代表 Apple 已成功创建地址。新候选先执行本轮短暂的目录核对；候选创建满 5 分钟后，后续计划若成功读取当前主号完整且身份匹配的 Apple 目录，仍未找到该地址，会自动清理本地仍未启用的待确认候选及其凭据（`APPLE_ALIAS_CANDIDATE_DISCARDED`），不调用 Apple 停用或删除接口；该轮结束，下一次计划恢复新建。既有卡住记录沿用创建时间处理，无需数据库迁移。5 分钟是本地恢复策略的宽限时间，不是 Apple 保证的同步时限。目录读取失败、限流或会话异常会保留候选并报告实际错误；远端存在但已停用时保留记录并报告 `APPLE_ALIAS_INACTIVE`，不会当作缺失丢弃。
